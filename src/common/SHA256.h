...
namespace std {
inline const char *data(const char *_ptr)
{
    return _ptr;
}
}
...
public:
    SHA256(T m): SHA256(m, std::size(m))
    {
    }

    SHA256(T m, std::size_t _size)
    {
        int l = (_size * 8);
        ...
        std::copy(std::data(m), std::data(m) + _size, std::back_inserter(message));
    ...
    operator std::string()
    {
        ...
        {
            char buffer[9];
            snprintf(buffer, std::size(buffer), "%.8x", H[i]);
            ss += buffer;
        }
